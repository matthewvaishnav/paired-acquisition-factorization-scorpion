#!/usr/bin/env python3
"""Analyze frozen five-fold SCORPION Paired-Acquisition Neural Factorization test results.

Each of the 48 original slides contributes test outcomes exactly once. Metrics
are averaged over five optimization seeds within slide, then Paired-Acquisition Neural Factorization minus
paired-reference contrasts are inferred over the 48 slide blocks.
"""
...
def split_indices(frame: pd.DataFrame):
    ...
    fit_slides = set(frame.iloc[fit]["slide_id"])
    test_slides = set(frame.iloc[test]["slide_id"])
    if fit_slides & test_slides:
        raise AnalysisError("Original-slide leakage between fit and test.")
    return fit, test
...
def test_maps(features, frame, test):
    ...
    for index, row in test_frame.iterrows():
        ...
        region_to_slide[region] = slide
    return maps, region_to_slide


def paired_slide_metrics(features, frame, test):
    maps, region_to_slide = test_maps(features, frame, test)
    cosine_rows = []
    retrieval_rows = []
    pair_summary = []
    ...
    slides = sorted(set(cosine["slide_id"]))
    slide_rows = []
    for slide in slides:
        slide_rows.append(
            {
                "slide_id": slide,
                "pair_cosine_average": float(
                    cosine.loc[cosine["slide_id"] == slide, "cosine"].mean()
                ),
                "pair_cosine_worst": float(cosine_by_pair.loc[slide].min()),
                "retrieval_top1_average": float(
                    retrieval.loc[retrieval["slide_id"] == slide, "correct"].mean()
                ),
                "retrieval_top1_worst": float(retrieval_by_pair.loc[slide].min()),
            }
        )
...
def main() -> None:
    ...
    for row in training.itertuples(index=False):
        ...
        paired, slide_paired = paired_slide_metrics(biological, frame, test)
        ...
        run_rows.append(
            {
                ...
                **paired,
                ...
                "feature_variance_nonzero_fraction": float(
                    np.mean(variances > 1e-12)
                ),
        ...
        if variant == "paired_acquisition_dep20":
            ...
            acquisition_paired, _ = paired_slide_metrics(acquisition, frame, test)
            factor_rows.append(
                {
                    "fold": fold,
                    "seed": seed,
                    "acquisition_scanner_probe": acquisition_probe[
                        "balanced_accuracy"
                    ],
                    "acquisition_tissue_retrieval": acquisition_paired[
                        "retrieval_top1_average"
                    ],
                    "acquisition_effective_rank": effective_rank(acquisition[test]),
                    "cross_covariance_rms": cross_covariance_rms(
                        biological, acquisition, test
                    ),
                }
            )
    ...
    slides = pd.DataFrame(slide_rows).sort_values(
        ["variant", "slide_id", "seed"]
    )
    factors = pd.DataFrame(factor_rows).sort_values(["fold", "seed"])
    runs.to_csv(args.out_dir / "raw_run_metrics.csv", index=False)
    slides.to_csv(args.out_dir / "raw_slide_metrics.csv", index=False)
    factors.to_csv(args.out_dir / "raw_factor_metrics.csv", index=False)

    counts = slides.groupby(["variant", "seed"])["slide_id"].nunique()
    if not (counts == 48).all():
        raise SystemExit("Every method/seed must evaluate all 48 slides exactly once.")

    metric_columns = list(METRICS)
    slide_means = (
        slides.groupby(["variant", "slide_id"], as_index=False)[metric_columns]
        .mean()
    )
    slide_means.to_csv(args.out_dir / "slide_seed_averaged_metrics.csv", index=False)
    paired = slide_means[slide_means["variant"] == "paired_reference"].set_index(
        "slide_id"
    )
    patho = slide_means[slide_means["variant"] == "paired_acquisition_dep20"].set_index(
        "slide_id"
    )
    if set(paired.index) != set(patho.index) or len(paired) != 48:
        raise SystemExit("Slide blocks are not matched across methods.")

    contrast_rows = []
    contrast_lookup = {}
    for metric_index, metric in enumerate(METRICS):
        differences = (patho.loc[paired.index, metric] - paired[metric]).to_numpy(float)
        lower, upper = bootstrap_ci(differences, seed=2026 + metric_index)
        p_value = sign_flip_p(differences, seed=3026 + metric_index)
        ...
        row = {
            ...
            "n_slide_blocks": len(differences),
    ...
    contrasts.to_csv(args.out_dir / "slide_blocked_contrasts.csv", index=False)
    ...
    factor_means = {
        key: float(value)
        for key, value in factors.drop(columns=["fold", "seed"]).mean().items()
    }
    ...
    success = {
        ...
        "mean_retrieval_noninferior_margin_0_02": mean_retrieval[
            "bootstrap_ci_025"
        ] >= -0.02,
        "worst_retrieval_noninferior_margin_0_02": worst_retrieval[
            "bootstrap_ci_025"
        ] >= -0.02,
        "mean_pair_cosine_ci_above_zero": mean_cosine["bootstrap_ci_025"] > 0,
        "worst_pair_cosine_ci_above_zero": worst_cosine[
            "bootstrap_ci_025"
        ] > 0,
        "all_biological_dimensions_nonzero": bool(
            runs.loc[
                runs["variant"] == "paired_acquisition_dep20",
    ...
    summary = {
        "n_unique_test_slides": 48,
        "n_seeds_per_slide": 5,
        "method_descriptive_means": method_means.to_dict("records"),
        "paired_acquisition_factor_means": factor_means,
...
if __name__ == "__main__":
    try:
        ...
        print(f"SCORPION CROSS-FOLD ANALYSIS FAILED: {exc}", file=sys.stderr)
